...
void gradient_descent_update_host_vectors(mlp_t mlp, mlp_gradient_compute_data gradient_compute_data, host_vector_t input, host_vector_t output_truth, device_vector_t device_in_buf, device_vector_t device_out_buf) {
...
void gradient_descent_update_device_vectors(mlp_t mlp, mlp_gradient_compute_data gradient_compute_data, device_vector_t input, device_vector_t output_truth, device_vector_t device_in_buf, device_vector_t device_out_buf) {
    // ASSERT_EQ(mlp.layers[0].in_dimension, input.size)
    // ASSERT_EQ(mlp.layers[mlp.num_layers-1].out_dimension, output.size)

    // // Forward computation
    // network_compute(mlp, gradient_compute_data.layer_outputs, input);
